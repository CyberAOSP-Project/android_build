...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^cyber_") ; then
        CYBER_BUILD=$(echo -n $1 | sed -e 's/^cyber_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $CYBER_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        CYBER_BUILD=
    fi
    export CYBER_BUILD
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the CyberAOSP GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/cyber/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/cyber/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
